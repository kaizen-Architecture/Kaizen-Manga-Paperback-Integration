...
var _Sources = (() => {
  ...
  // src/KaizenManga/KaizenManga.ts
  var KaizenManga_exports = {};
  __export(KaizenManga_exports, {
    KaizenManga: () => KaizenManga,
    KaizenMangaInfo: () => KaizenMangaInfo
  });
  ...
  function getCoverUrl(metadata, host, token) {
  ...
  var KaizenManga = class extends import_types.Source {
    ...
    async getSearchTags() {
      try {
        ...
        const tags = Array.from(genreSet).sort().map(
          (g) => App.createTag({ id: g, label: g })
        );
        return [
